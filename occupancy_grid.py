...
class OccupancyGrid:
    ...
    def display_plt(self, robot_pose, goal=None, traj=None, goals=None, active_goal_index=None):
        """
        ...
        if goals is not None:
            ...
            if goals_array.size > 0:
                plt.scatter(goals_array[:, 0], goals_array[:, 1], 18, c='yellow')
                for idx, waypoint in enumerate(goals):
                    plt.text(waypoint[0] + 4, waypoint[1] + 4, str(idx), color='black', fontsize=8)

        if goal is not None:
            plt.scatter(goal[0], goal[1], 28, c='white')

        if active_goal_index is not None and goals is not None and 0 <= active_goal_index < len(goals):
            active_goal = goals[active_goal_index]
            plt.scatter(active_goal[0], active_goal[1], 40, c='red')
    ...
    def display_cv(self, robot_pose, goal=None, traj=None, goals=None, active_goal_index=None):
        """
        Screen display of map and robot pose,
        ...
        if goals is not None:
            for idx, waypoint in enumerate(goals):
                pt_x, pt_y = self.conv_world_to_map(waypoint[0], waypoint[1])
                if pt_x < 0 or pt_x >= self.x_max_map or pt_y < 0 or pt_y >= self.y_max_map:
                    continue
                point = (int(pt_x), self.y_max_map - int(pt_y))
                color = (0, 255, 255)
                radius = 4
                if active_goal_index is not None and idx == active_goal_index:
                    color = (0, 0, 255)
                    radius = 6
                cv2.circle(img_color, point, radius, color, -1)
                cv2.putText(img_color, str(idx), (point[0] + 6, point[1] - 6),
